...
public class Job {
	...
	public ArrayList <String> showMyDuties(){
		
		return jduties; 
	}
	
	public void populateMyDuties(int personID){
		this.jduties = null; 
		//Populates the jduties arraylist with items from the database
		String selectSQL = "select * from jobduties where jobid = ?  and personid = ?"; 
		...
		try{
			...
			while(rs.next()){
				jduties.add(rs.getString("jobduty"));
			}
